...
# failed_defense_augmented_dna.jsonl 질문으로 LoRA OFF / ON 답변만 생성할 때 사용
FAILED_JSONL_PATH = "./failed_defense_augmented_dna.jsonl"
FAILED_JSONL_QUESTION_FIELD = "question"          # "question" 또는 "original_question"
ANSWER_ONLY_FAILED_JSONL = True                   # True면 clustering 평가 없이 답변만 생성
ANSWER_OUTPUT_TXT = "failed_dna_lora_on_off_answers.txt"
ANSWER_OUTPUT_JSONL = "failed_dna_lora_on_off_answers.jsonl"
MAX_ANSWER_SAMPLES = None                         # 일부만 테스트하려면 숫자 지정 ex) 30
...
# =========================================================
# failed_defense_augmented_dna.jsonl 로더 / 답변 저장
# =========================================================
def load_failed_jsonl_questions(jsonl_path, question_field="question", max_samples=None):
    """
    failed_defense_augmented_dna.jsonl에서 질문만 읽어온다.
    기본은 augmented된 question 필드를 사용한다.
    원본 질문으로 생성하고 싶으면 question_field="original_question"로 바꾸면 된다.
    """
    ...
    with open(jsonl_path, "r", encoding="utf-8") as f:
        for line_idx, line in enumerate(f):
            ...
            try:
                ...
                print(f"⚠️ JSON 파싱 실패: line={line_idx + 1}, error={e}")
                continue

            prompt = item.get(question_field)
            if not prompt:
                # question_field가 없거나 비어 있으면 original_question → question 순서로 fallback
                prompt = item.get("original_question") or item.get("question")

            if not prompt:
                print(f"⚠️ 질문 필드 없음: line={line_idx + 1}")
                continue
            ...
            meta.append({
                ...
                "base_response_in_file": item.get("base_response", ""),
            })

            if max_samples is not None and len(prompts) >= max_samples:
                break

    print(f"failed JSONL 로드: {len(prompts)}개")
    print(f"사용 질문 필드: {question_field}")
    return prompts, meta


def save_lora_on_off_answers(
    model,
    tokenizer,
    prompts,
    meta,
    txt_path="failed_dna_lora_on_off_answers.txt",
    jsonl_path="failed_dna_lora_on_off_answers.jsonl",
    max_new_tokens=256,
):
    """
    각 질문에 대해:
      - LoRA OFF: base model 답변
      - LoRA ON : adapter 적용 답변
    을 생성해서 txt와 jsonl 두 파일로 저장한다.
    """
    with open(txt_path, "w", encoding="utf-8") as txt_f, \
         open(jsonl_path, "w", encoding="utf-8") as jsonl_f:

        txt_f.write("=" * 100 + "\n")
        txt_f.write("FAILED DEFENSE AUGMENTED DNA - LoRA OFF / ON ANSWER REPORT\n")
        txt_f.write("=" * 100 + "\n\n")
        txt_f.write(f"TOTAL QUESTIONS: {len(prompts)}\n")
        txt_f.write(f"SOURCE JSONL   : {FAILED_JSONL_PATH}\n")
        txt_f.write(f"QUESTION FIELD : {FAILED_JSONL_QUESTION_FIELD}\n\n")

        for i, prompt in enumerate(prompts, start=1):
            before_answer, after_answer = generate_answer_pair(
                model,
                tokenizer,
                prompt,
            ...
            item_meta = meta[i - 1] if i - 1 < len(meta) else {}

            record = {
                "idx": i - 1,
                "risk_area": item_meta.get("risk_area", ""),
                "question_used": prompt,
                "question": item_meta.get("question", ""),
                "original_question": item_meta.get("original_question", ""),
                "reference_response": item_meta.get("reference_response", ""),
                "base_response_in_file": item_meta.get("base_response_in_file", ""),
                "base_model_answer_lora_off": before_answer.strip(),
                "lora_model_answer_lora_on": after_answer.strip(),
            }
            jsonl_f.write(json.dumps(record, ensure_ascii=False) + "\n")

            txt_f.write("=" * 100 + "\n")
            txt_f.write(f"[{i}/{len(prompts)}] idx={i - 1}\n")
            txt_f.write(f"RISK_AREA:\n{record['risk_area']}\n\n")
            txt_f.write("QUESTION_USED:\n")
            txt_f.write(prompt.strip() + "\n\n")

            if record["original_question"] and record["original_question"] != prompt:
                txt_f.write("ORIGINAL_QUESTION:\n")
                txt_f.write(record["original_question"].strip() + "\n\n")

            txt_f.write("BASE MODEL ANSWER (LoRA OFF):\n")
            txt_f.write(before_answer.strip() + "\n\n")
            txt_f.write("LORA MODEL ANSWER (LoRA ON):\n")
            txt_f.write(after_answer.strip() + "\n\n")

            if i % 10 == 0:
                print(f"  답변 생성 {i}/{len(prompts)}...")
...
if __name__ == "__main__":
    ...
    # 2. failed_defense_augmented_dna.jsonl 질문으로 LoRA OFF/ON 답변만 생성
    if ANSWER_ONLY_FAILED_JSONL:
        print("\n========== failed_defense_augmented_dna.jsonl 답변 생성 ==========")
        eval_prompts, eval_meta = load_failed_jsonl_questions(
            FAILED_JSONL_PATH,
            question_field=FAILED_JSONL_QUESTION_FIELD,
            max_samples=MAX_ANSWER_SAMPLES,
        )
        save_lora_on_off_answers(
            model=model,
            tokenizer=tokenizer,
            prompts=eval_prompts,
            meta=eval_meta,
            txt_path=ANSWER_OUTPUT_TXT,
            jsonl_path=ANSWER_OUTPUT_JSONL,
            max_new_tokens=256,
        )
        raise SystemExit(0)
