...
def build_category_conditioned_messages(prompt, pred_label=None):
    messages = []
    if pred_label is not None and int(pred_label) in category_instructions:
        messages.append({"role": "system", "content": category_instructions[int(pred_label)]})
...
def generate_answer_pair(model, tokenizer, prompt, pred_label=None, max_new_tokens=256):
    # LoRA OFF: baseline 비교용이므로 카테고리 지침 없이 생성
    ...
    # LoRA ON: 예측된 카테고리 지침을 system prompt로 넣어서 생성
...
if __name__ == "__main__":
    ...
    plot_after_tsne_with_failed_overlay(
        ...
        save_path="after_lora_train_tsne_plot_with_failed_json_llama.png",
    )
    ...
    save_failed_json_answers(
        ...
        max_new_tokens=MAX_NEW_TOKENS,
        save_txt="failed_json_lora_on_off_answers_llama.txt",
        save_jsonl="failed_json_lora_on_off_answers_llama.jsonl",
    )
